...
\end{equation}
%
\new{Note to reviewers: In equation~\ref{eq:base} $N/2$ changed to
$\lfloor{(N+1)/2\rfloor}$.} From equation~\ref{eq:filtapprox} it can be
seen that the filter method requires $N$ evaluations at each $r$. For example,
to calculate the frequency domain result for 100 offsets with a 201\,pt filter
requires 20,100 evaluations in the wavenumber domain. This is why the DLF often
uses interpolation to minimize the required evaluations, either for $F(r)$ in
what is referred to as \emph{lagged convolution DLF}, or for $f(l)$, which we
here call \emph{splined DLF}.
...
influence the outcome, and often a \old{good}\new{even better} filter is found
by sheer luck of choosing the right starting parameters\new{, as can be seen in
Figure~\ref{fig:201c} (h). However, once you are in this zone every filter is a
very good filter}. Each of the input parameters has an effect on the outcome.
It also depends heavily on the transform pairs $f_\mr{I;C}$; functions that
decay rapidly are generally better, as noted by earlier authors
...
\texttt{empymod}\old{, called \texttt{empyscripts},} from version
\old{0.2.0}\new{1.7.0} onwards.
...
We thank the reviewers for valuable inputs not only regarding the manuscript,
but also regarding the code and its documentation.
