...
\subsection{Design}
Figure~\ref{fig:201} shows the solution spaces of \old{four}\new{eight}
consecutive inversion runs\old{, where each run is a focus on a subsection of
  the previous run, indicated by the red square. The filter length in this case
is $N=201$, and}\new{where} the theoretical transform pairs
\old{are} as given, e.g., in \cite{USGS.75.Anderson}\old{:}\new{,}
%
\begin{align}
...
%
where \new{used; } $a$ was set to 5. In the algorithm you can provide one pair
...
\new{The top row shows in subplots (a) to (d) four different inversion runs
for $N=51$, 101, 201, and 401 for a wide range of spacing and shift values.
...
$-(N-1)/2$ to $(N-1)/2$. However, this is only true if the shift value is
$\delta = 0$. If your shift value is not zero then there is no advantage in
having an odd point filter.}

\new{The bottom row shows four consecutive inversion runs for $N=201$, where
each run is a focus on a subsection of the previous run, indicated by the red
square.} From the low resolution overview runs (e) and (f) it looks like this
is a standard, straight-forward minimization problem. However, from the more
detailed results in (g) and (h) it becomes obvious that it is a minimization
...
choosing the right starting parameters\new{, as can be seen in
Figure~\ref{fig:201b} (h). However, once you are in this zone every filter is a
