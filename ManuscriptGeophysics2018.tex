...
rewritten, via recurrence relations, using only\old{use} these two.
...
center-frequency GPR data using a 401 \old{point}\new{pt} filter that was
...
%
\plot*{Figure-1-bw}{width=.9\textwidth}{(a) A regular \new{right-hand-side
  (}RHS\new{)} curve of a transform pair with a purely decaying function.
  Minimum amplitude or maximum $r$ yield the same result in this scenario.
  Using a relative error criteria is more stable than just the absolute minimum
  amplitude. (b) A 1D model for $f=200\,$MHz. Here, the maximum $r$ provides a
  better criteria for the inversion.}
%
...
\old{red} square. From the low resolution overview runs (a) and (b) it looks
like this is a standard, straight-forward minimization problem. However, from
the more detailed results in (c) and (d) it becomes obvious that it is a
...
\plot*{Figure-3-bw}{width=\textwidth}{%
  ...
  indicated by the \old{red} square. The more in detail we obtain the solution
  space, the more random appears to be the distribution.}%
%

Figure~\ref{fig:Figure-4-bw} shows in (a) the filter values for $J_0$ and $J_1$
of the best 201 \old{point}\new{pt} filter obtained this way, and in (b) its
right-hand-side solution. The black dots indicate negative values, from which
it can \new{be} seen that adjacent values are often alternating between
...
to marine CSEM models \new{as} used in \cite{GP.07.Kong}, \new{in}
\cite{GEO.12.Key}, \old{as well as}\new{and} for a land EM case.
...
\plot*{Figure-6-bw}{width=\textwidth}{(a) The canonical marine CSEM model of
  ...
  surface. \old{The new filter has generally relative errors which are several
  orders of magnitude lower than the other filters.} \new{The new filter shows
  relative errors that are generally several orders of magnitude lower than
...
model \new{(2\,km water depth)}, a shallow water model \new{(400\,m water
depth)}, and a land model. The models consist of air (upper halfspace), a water
...
\plot*{Figure-9}{width=\textwidth}{GPR example for the model given in (a) using
  \texttt{empymod} with (b) \old{the} adaptive quadrature (QUAD) or with (c) a
  2001\,pt digital linear filter\old{ (b)}. QUAD took about 11\,h 27\,min to
  calculate, whereas DLF took 2\,min 10\,s in parallel on 4 nodes, hence under
  9\,min in total.}
%

Figure~\ref{fig:Figure-10-bw} shows in (a)-(c) the real \new{and imaginary}
part\new{s} of the frequency domain results and in (d)-(f) the time domain
results for offsets of 0.2\,m, 2.0\,m, and 3.0\,m.
%
\plot*{Figure-10-bw}{width=\textwidth}{The real\old{-part} \new{and imaginary
  parts of the} frequency-domain \new{(a-c)} and the time-domain \new{(d-f)}
  responses for \new{horizontal} offsets of (a/d) 0.2\,m, (b/e) 2.0\,m, and
  (c/f) 3.0\,m.}
%
...
\plot*{Figure-11-bw}{width=\textwidth}{The real and imaginary parts of the
  ...
  (a) with swapped first and second layers for \new{horizontal} offsets of (a)
...
resistivity and \old{em}\new{EM} sounding curves: Geoexploration, {\bf 18},
